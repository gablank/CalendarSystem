...
public class ClientHandler extends Network {
	...
	protected void handleMessage(JSONObject message) throws IOException {
		if (message.has("type")) {
			...
			// && message.has("title") && message.has("description") &&
			// message.has("start") && message.has("end") &&
			// message.has("meetingPlace") && message.has("attendants") &&
			// message.has("owner") && message.has("meetingRoom")) {
			//
